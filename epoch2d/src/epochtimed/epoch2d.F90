...
#endif
#ifndef PERFMON2D
#error  "Do not use this without PERFMON2D!"
#endif
  USE perfmon, only: init_perfmon, perfmon_update, &
  ...
  DO
    CALL perfmon_start(c_perfid_timestep)
    ...
    CALL perfmon_start(c_perfid_fieldsolve1)
    CALL update_eb_fields_half
    CALL perfmon_stop(c_perfid_fieldsolve1)
    ...
    IF (push) THEN
      ...
      CALL perfmon_start(c_perfid_partpush)
      CALL push_particles
      CALL perfmon_stop(c_perfid_partpush)
      ...
      IF (use_particle_lists) THEN
        ...
        CALL perfmon_start(c_perfid_part2grid)
        CALL reorder_particles_to_grid
        CALL perfmon_stop(c_perfid_part2grid)

        ! call collision operator
        CALL perfmon_start(c_perfid_collision)
    ...
    CALL perfmon_start(c_perfid_fieldsolve2)
    CALL update_eb_fields_final
    CALL perfmon_stop(c_perfid_fieldsolve2)

    CALL moving_window
    CALL perfmon_stop(c_perfid_timestep)
    CALL perfmon_update
  END DO
