...
CONTAINS
  ...
  SUBROUTINE pack_particle(array, a_particle)
    ...
    cpos = 1
#ifdef BOUND_HARMONIC
    array(cpos:cpos+c_ndims)   = a_particle%part_pos
    cpos = cpos+3
#else
...
#endif
#ifdef BOUND_HARMONIC
    array(cpos) = a_particle%part_ip(1)
    array(cpos+1) = a_particle%part_ip(2)
    array(cpos+2) = a_particle%part_ip(3)
    cpos = cpos+3
#endif
  ...
  SUBROUTINE unpack_particle(array, a_particle)
    ...
    cpos = 1
#ifdef BOUND_HARMONIC
    a_particle%part_pos = array(cpos:cpos+c_ndims)
    cpos = cpos+3
#else
...
#endif
#ifdef BOUND_HARMONIC
    a_particle%part_ip(1) = array(cpos)
