...
CONTAINS
  ...
  SUBROUTINE bremsstrahlung_update_optical_depth
    ...
    IF (use_plasma_screening) THEN
      ...
      DO i = 1-ng, nx+ng
        IF (grid_num_density_electron(i) < 1.0e-10_num) THEN
          grid_root_temp_over_num(i) = 0.0_num
        ELSE IF (grid_temperature_electron(i) < 1.0e-10_num) THEN
          grid_root_temp_over_num(i) = 0.0_num
        ELSE
          grid_root_temp_over_num(i) = &
              SQRT(grid_temperature_electron(i) &
              / grid_num_density_electron(i))
        END IF
      END DO
