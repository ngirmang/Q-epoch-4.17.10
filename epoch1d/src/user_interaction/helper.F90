  ...
  SUBROUTINE load_particles(species, load_list)
    ...
    IF (npart_per_cell > 0) THEN
#ifdef BOUND_HARMONIC
      IF (species%dir_nparts > 0) THEN
        cdx = dx / REAL(species%dir_nparts,num)
      END IF
      ...
      DO ix = ix_min, ix_max
        ...
        IF      (species%dir_nparts == 0) THEN
          cxmin = 0
        ELSE IF (species%dir_nparts == 1) THEN
          cxmin = x(ix)
        ELSE IF (MOD(species%dir_nparts,2_i8) == 0) THEN
          cxmin = xb(ix) + dx / (species%dir_nparts*2)
        ELSE
          cxmin = xb(ix)
        END IF
#endif !BOUND_HARMONIC
...
#ifdef BOUND_HARMONIC
          IF (species%dir_nparts > 0) THEN
            current%part_pos(1) = cxmin + ipart*cdx
          ELSE
            current%part_pos(1) = x(ix) + (random() - 0.5_num) * dx
          END IF
          current%part_pos(2:3) = 0.0_num
...
#include "particle_to_grid.inc"
...
#endif !PARTICLE_SHAPE_TOPHAT
#else !else ifndef BOUND_HARMONIC
#ifdef PARTICLE_SHAPE_TOPHAT
    ...
    IF (species%fill_ghosts .AND. use_injectors) THEN
      ...
      DO WHILE(ASSOCIATED(current))
        next => current%next
#ifdef BOUND_HARMONIC !sigh 
        IF (current%part_pos(1) < x0 .OR. current%part_pos(1) >= x1) THEN
#else
