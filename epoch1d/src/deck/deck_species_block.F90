...
MODULE deck_species_block
  ...
  REAL(num), DIMENSION(3) :: species_harmonic_om
...
CONTAINS
  ...
  FUNCTION species_block_handle_element(element, value) RESULT(errcode)
...
#endif
#ifdef BOUND_HARMONIC
    IF (str_cmp(element, 'harmonic_omega_x')) THEN
      species_harmonic_om(1) = as_real_print(value, element, errcode)
      RETURN
    END IF
    IF (str_cmp(element, 'harmonic_omega_y')) THEN
      species_harmonic_om(2) = as_real_print(value, element, errcode)
      RETURN 
    END IF
    IF (str_cmp(element, 'harmonic_omega_z')) THEN
      species_harmonic_om(3) = as_real_print(value, element, errcode)
      RETURN 
    END IF
    IF (str_cmp(element, 'harmonic_gamma_x')) THEN
      species_harmonic_gm(1) = as_real_print(value, element, errcode)
      RETURN 
    END IF
    IF (str_cmp(element, 'harmonic_gamma_y')) THEN
      species_harmonic_gm(2) = as_real_print(value, element, errcode)
      RETURN 
    END IF
    IF (str_cmp(element, 'harmonic_gamma_z')) THEN
      species_harmonic_gm(3) = as_real_print(value, element, errcode)
      RETURN 
    END IF
    IF      (str_cmp(element, 'harmonic_omega')) THEN
      species_harmonic_om = as_real_print(value, element, errcode)
      RETURN
    ELSE IF (str_cmp(element, 'harmonic_gamma')) THEN
      species_harmonic_gm = as_real_print(value, element, errcode)
      RETURN
    END IF
    ...
    IF      (str_cmp(element, 'xdir_npart_per_cell')) THEN
       species_dir_nparts = as_integer_print(value, element, errcode)
       RETURN
    END IF
    ...
    END IF
#ifdef BOUND_HARMONIC
    IF (str_cmp(element, 'drive_x')) THEN
      CALL initialise_stack(species_list(species_id)%drive_function(1))
      CALL tokenize(value, species_list(species_id)%drive_function(1), &
           errcode)
      tmp = evaluate(species_list(species_id)%drive_function(1), errcode)
      ...
      END IF
    ELSE IF (str_cmp(element, 'drive_y')) THEN
      CALL initialise_stack(species_list(species_id)%drive_function(2))
      CALL tokenize(value, species_list(species_id)%drive_function(2), &
           errcode)
      tmp = evaluate(species_list(species_id)%drive_function(2), errcode)
      IF (errcode /= c_err_none) THEN
        IF (rank == 0) THEN
          DO iu = 1, nio_units ! Print to stdout and to file
            io = io_units(iu)
        ...
        CALL abort_code(errcode)
      END IF
    ELSE IF (str_cmp(element, 'drive_z')) THEN
      CALL initialise_stack(species_list(species_id)%drive_function(3))
      CALL tokenize(value, species_list(species_id)%drive_function(3), &
           errcode)
      tmp = evaluate(species_list(species_id)%drive_function(3), errcode)
      IF (errcode /= c_err_none) THEN
        IF (rank == 0) THEN
          DO iu = 1, nio_units ! Print to stdout and to file
            io = io_units(iu)
...
#endif
#ifdef BOUND_HARMONIC
    ! check species ndir, if > 0, just set to npart_per_cell
    DO i = 1, n_species
      IF (species_list(i)%dir_nparts > 0 &
           .AND. FLOOR(species_list(i)%npart_per_cell,i8) &
           /= species_list(i)%dir_nparts) THEN
        PRINT '(a)', '*** WARNING ***'
        PRINT '(a,I3)', 'xdir_npart_per_cell do not match for species ', i
        PRINT '(a,I3,a,I4,a)', &
             'xdir = ', species_list(i)%dir_nparts, '/=', &
             FLOOR(species_list(i)%npart_per_cell,i8), '= npart_per_cell'
        PRINT '(a)', 'setting xdir to npart_per_cell'
        species_list(i)%dir_nparts = FLOOR(species_list(i)%npart_per_cell,i8)
      END IF
    END DO
#endif    
  ...
  FUNCTION create_species_number_from_name(name)
    ...
    CALL grow_array(dir_nparts, n_species)
#endif
