...
#ifdef SIMPLE_ADK
            scaled_epart = adk_scaling(current_state) / e_part_mag
            rate = ionisation_constant(current_state) &
                 * (scaled_epart)**effective_n_exponent(current_state) &
                 * EXP(-scaled_epart/3.0_num)
            ! IF (step > 1 .and. current_state == 4) THEN
            !   IF (scaled_epart > 0.0 &
            !        .and. current%part_pos(3) >  -1.000e-10_num &
            !        .and. current%part_pos(3) <   1.000e-10_num &
            !        .and. current%part_pos(2) > -49.000e-09_num &
            !        .and. current%part_pos(2) <  49.000e-09_num &
            !        .and. current%part_pos(1) > -15.049e-06_num &
            !        .and. current%part_pos(1) < -14.951e-06_num ) THEN
            !     PRINT "(A, I6)", "step=", step
            !     PRINT "(A, ES9.2)", " e_part_mag=",   e_part_mag
            !     PRINT "(A, ES9.2)", " scaled_epart=", scaled_epart
            !     PRINT "(A,ES9.2,A,ES9.2,A,ES9.2)", &
            !          " rate*time_left = ",rate,"*",time_left,"=", &
            !          rate*time_left
            !   END IF
            ! END IF
#else
