...
!                   1234567890123456789012345678901234567890123456789012
    ...
    DO iprefix = 1,SIZE(file_prefixes)
      ...
      IF (eps_stored) THEN
        CALL write_field(c_dump_eps_x, code, 'epsx', &
             ...
             c_stagger_centre, epsx)
        CALL write_field(c_dump_eps_y, code, 'epsy', &
             'Dielectric Constant in y', 'e0', &
             c_stagger_centre, epsy)
  ...
  SUBROUTINE write_field(id, code, block_id, name, units, stagger, array)
    ...
    !! added
    !REAL(num), DIMENSION(2,c_ndims) :: oldranges
    !! end added
      ...
      DO ispecies = 1, n_species
        ...
        DO k = 1, rnz
          ...
          DO j = 1, rny
            ii = sub%n_start(1) + 1
            DO i = 1, rnx
              reduced(i,j,k) = media_density(ii,jj,kk,im)
              ii = ii + sub%skip_dir(1)
            END DO
            jj = jj + sub%skip_dir(2)
          END DO
    ...
    ELSE ! not skipped
      DO ispecies = 1, n_species
        ...
        IF (dump_part) THEN
          ...
          CALL sdf_write_plain_variable(sdf_handle, TRIM(temp_block_id), &
              TRIM(temp_name), TRIM(units), new_dims, stagger, temp_grid_id, &
              media_density(i0:i1,j0:j1,k0:k1,im), rsubtype, rsubarray, convert)
          sub%dump_field_grid = .TRUE.
        ELSE
          CALL sdf_write_plain_variable(sdf_handle, TRIM(temp_block_id), &
              TRIM(temp_name), TRIM(units), dims, stagger, 'grid', &
