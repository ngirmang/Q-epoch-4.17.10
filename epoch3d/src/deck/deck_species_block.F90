...
!end BOUND_HARMONIC
  ...
  FUNCTION species_block_handle_element(element, value) RESULT(errcode)
    ...
    IF      (str_cmp(element, 'xdir_npart_per_cell')) THEN
      ...
      RETURN
    ELSE IF (str_cmp(element, 'ydir_npart_per_cell')) THEN
      species_dir_nparts(2) = as_integer_print(value, element, errcode)
      RETURN
    ELSE IF (str_cmp(element, 'zdir_npart_per_cell')) THEN
...
! end BOUND_HARMONIC
  ...
  FUNCTION species_block_check() RESULT(errcode)
    ...
    INTEGER :: i, io, iu
#ifdef BOUND_HARMONIC
    INTEGER(i8) :: n, ndir(3), npc !you might be an npc if
#endif
    ...
    DO i = 1, n_species
      ...
      IF ( npc == -1 ) CYCLE
      n = species_list(i)%dir_nparts(1)
      n = n*species_list(i)%dir_nparts(2)
      n = n*species_list(i)%dir_nparts(3)

      IF (npc /= n) THEN
        PRINT '(a)', '*** ERROR ***'
        PRINT '(a,I3)', '*dir_npart_per_cell do not match for species ', i
        PRINT '(a,I3,a,I3,a,I3,a,I4,a,I4,a)', &
             'xdir*ydir*zdir = ', &
             species_list(i)%dir_nparts(1), '*', species_list(i)%dir_nparts(2),&
             '*', species_list(i)%dir_nparts(3), '=', n, '/=', &
             npc, '= npart_per_cell'
        errcode = c_err_missing_elements
      END IF
    END DO
