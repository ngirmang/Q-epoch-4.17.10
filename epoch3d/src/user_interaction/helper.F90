...
CONTAINS
  ...
  SUBROUTINE load_particles(species, load_list)
    ...
    INTEGER :: iu, io
#ifdef BOUND_HARMONIC
    INTEGER(i8) :: cnx, cny, cnz
    REAL(num), DIMENSION(3) :: cdx, cmin
#endif
    ...
    IF (npart_per_cell > 0) THEN
      ...
      IF (ALL(species%dir_nparts > 0)) THEN
        cdx = (/dx,dy,dz/) / REAL(species%dir_nparts,num)
      END IF
...
#ifdef BOUND_HARMONIC
          IF (ALL(species%dir_nparts > 0)) THEN
            cnx = ipart / (species%dir_nparts(2)*species%dir_nparts(3))
            i   = MOD(ipart, species%dir_nparts(2)*species%dir_nparts(3))
            cny = i / species%dir_nparts(3)
            cnz = MOD(i, species%dir_nparts(3))
            current%part_pos = cmin + cdx*(/cnx,cny,cnz/)
          ELSE
...
#include "particle_to_grid.inc"
  ...
  FUNCTION pos_to_cell(r)
    ...
    REAL(num) dr(3)
    dr = r - (/x_grid_min_local, y_grid_min_local, z_grid_min_local /)
