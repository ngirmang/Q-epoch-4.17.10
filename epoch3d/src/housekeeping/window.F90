...
CONTAINS
  ...
  SUBROUTINE insert_particles
    ...
    INTEGER(i8) :: cnx, cny, cnz, n
    REAL(num) :: cdx, cdy, cdz, cymin, czmin
#endif
    ...
    DO ispecies = 1, n_species
      ...
      IF     (species%dir_nparts(1) <= 0) THEN
        ...
        cdx = dx
      ELSEIF (species%dir_nparts(1) == 1) THEN
        x0  = x_grid_max
        cdx = dx
      ELSE
      ...
      DO iz = 1, nz
        DO iy = 1, ny
          ...
          IF   (species%dir_nparts(3) <= 1) THEN
            czmin = zb(iz)
            cdz = dz
          ELSE
            czmin = zb(iz) + dz / (species%dir_nparts(3)*2.0_num)
            cdz   = dz / REAL(species%dir_nparts(3),num)
          END IF          
#endif !BOUND_HARMONIC
          ...
          DO ipart = 1, npart_per_cell + n_frac
            ...
            IF (ANY(species%dir_nparts > 0)) THEN
              ...
              n   = MOD(ipart, species%dir_nparts(2)*species%dir_nparts(3))
              cny = n / species%dir_nparts(3)
              cnz = MOD(n, species%dir_nparts(3))
              
              cell_frac_y = cdy*cny
              cell_frac_z = cdz*cnz
              current%part_pos(1) = x0    + cdx*cnx
              current%part_pos(2) = cymin + cell_frac_y
              current%part_pos(3) = czmin + cell_frac_z
            ELSE
