...
CONTAINS
  ...
  SUBROUTINE create_subset_subtypes(subset_index)
    ...
    IF (ranges(2,3) - ranges(1,3) <= c_tiny) proc_outside_range = .TRUE.
  ...
  FUNCTION cell_local_ranges(ranges)
    ...
    DO idim = 1, c_ndims
      ...
      !!added
      !IF (ABS(ranges(1,idim)) < 1e-20) ranges(1,idim) = 0.0_num
      !IF (ABS(ranges(2,idim)) < 1e-20) ranges(2,idim) = 0.0_num
      
      !!endadded
      dd = dir_d(idim)
      lower_posn = dir_grid_min(idim)
      !!added
      !IF (ABS(lower_posn) < 1e-20) lower_posn = 0.0_num
      !
      !!endadded
